---
title: 'Lab 8, Part 2: Time Series Exploration with tsibble'
author: "Shuying Yu"
date: "2/24/2022"
...
library(lubridate)
library(tsibble)
library(feasts) #time series stats
library(slider)
```
...
```{r}
#Convert to tsibble: df with special characteristics that it is a time series
toolik_ts <- toolik %>% 
  
  #Take MDY of data column into propoer data column
  #Date rather than character column
  mutate(date = lubridate::mdy(date)) %>% 
  
  #Class not time series yet
  #index is what's used for later, use as time series index
...
toolik_month <- toolik_ts %>% 
  ...
  #In df we group by then summarize
  #Here we use index_by which is specific for time series tibbles
  #We index by year and month, create new column yr_mo
  index_by(yr_mo = ~yearmonth(.)) %>% 
  
  #Summarize omnthly mean temp
...
toolik_month %>% 
  ...
  #Extract year using lubridate on the x-axis
  ggplot(aes(x = year(yr_mo), y = monthly_mean_temp)) +
  geom_line() +
  
  #Use facet wrap to break it up by month
...
Average temperatured across the entire time period
...
Can still use same type of functions we've been using with regular tibbles and dataframes.
...
toolik_ts %>% 
  ...
  #Great for filtering between specific ranges
  filter_index("2015-01-01" ~ .) %>% 
  
  #From feasts packages, assum want dates on x-axis
...
Subseries plot, how it changes between certain windows
...
#Create window around the 100 observations to create moving average (-/+ 1)
 
# Show the series based on values +2 and -2 from each observation (5 obser total)
...
#Plot after pivot
combo2 <- tibble(time = 1:100, test, w5, w11, w19) %>% 
  pivot_longer(cols = test:w19, names_to = "model", values_to = "value")
ggplot(data = combo2) +
...
ggplot() +
  ...
  #Based on moving average 15 day window
  geom_line(data = roll_toolik_15, aes(x = date, y = ma_15d), color = "orange") +
  
  #Based on moving average 30 day window
  geom_line(data = roll_toolik_61, aes(x = date, y = ma_61d), color = "blue") +
  theme_minimal()
 
```

Grey higher extremes above and below, but smoothed it out with averages, We see more seasonality more cleanly with the moving averages.

# Autocorrelation function

We'll look at outcomes for both daily lags (yikes) and monthly lags (cool). Normally wouldn't do this on a day-to-day basis because the weather yesterday will very similar to today and the next day.

For month, the ones closer to each other are more similar than months that are farther in lag.

```{r}
#Use feasts package for auto plotting ACF 
toolik_ts %>%
  ACF(mean_airtemp) %>%
  autoplot()
